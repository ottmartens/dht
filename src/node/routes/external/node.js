...
const { id, successor } = require('../../nodeData');
...
module.exports = async (req, res) => {
	...
	if (!isTarget && target !== id) {
		...
		const isSuccessorTarget =
			(target > id && target < successor) ||
			(id > successor && (target > id || target < successor)); //Case 2
		try {
			const response = await axios.get(
				`http://localhost:${3000 + successor}/node?target=${target}&jumps=${
					jumps + 1
				}&isSuccessorTarget=${isSuccessorTarget}`
			);
